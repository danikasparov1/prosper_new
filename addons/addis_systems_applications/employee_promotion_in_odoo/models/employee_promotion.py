...
class EmployeeDemotion(models.Model):
    """This model is necessary for adding employee demotion details in the employee module"""
    _name = 'employee.demotion'
    _description = 'Employee Demotion'
    _inherit = ['mail.thread', 'mail.activity.mixin']
    _rec_name = 'demotion_name'

    demotion_name = fields.Text(required=True, string='Demotion Name', help='Demotion name')
    employee_id = fields.Many2one('hr.employee', string='Employee', required=True, help='Name of employee')
    contract_id = fields.Many2one('hr.contract', string='Contract', help='Contract of employee', domain="[('employee_id', '=', employee_id)]")
    job_title_id = fields.Many2one('hr.job', string='Old Designation', help='Previous job of employee')
    job_salary = fields.Float(string='Previous Salary', required=True, help='Previous job salary')
    demotion_date = fields.Date(string='Demotion Date', default=fields.Date.today(), help='Date of demotion')
    demotion_type_id = fields.Many2one('demotion.type', string='Demotion Type', required=True, help='Demotion type')
    new_designation_id = fields.Many2one('hr.job', string='New Designation', required=True, help='New designation of employee')
    new_salary = fields.Float(string='New Salary', required=True, help='New salary')
    description = fields.Text(string='Description', help='Description of the demotion')

    @api.model
    def create(self, vals):
        """It checks if the new salary is less than the old salary, raising a UserError if it is not the case."""
        res = super(EmployeeDemotion, self).create(vals)
        employee = self.env['hr.employee'].browse(res.employee_id.id)
        employee.write({
            'demotion_ids': [(4, res.id)],
            'job_id': res.new_designation_id.id
        })
        if res.job_salary <= res.new_salary:
            raise UserError("New Salary should be lower than Old Salary")
        return res

    @api.onchange('employee_id')
    def _onchange_employee_id(self):
        """This method is called when the employee_id field is changed.
           It searches for an open HR contract history record for the selected employee and sets the contract_id and job_salary fields based on the corresponding values in the contract.
           If no open contract is found, the fields are left blank."""
        self.job_title_id = self.employee_id.job_id.id
        contract = self.env['hr.contract.history'].search(
            [('employee_id', '=', self.employee_id.id),
    ...
    def write(self, vals):
        """ Override the default write method to check if the new salary is less than the current job salary.
            If the new salary is not less, raise a UserError with a warning message."""
        res = super(EmployeeDemotion, self).write(vals)
        if self.job_salary <= self.new_salary:
            raise UserError("New Salary should be lower than Old Salary")
        return res