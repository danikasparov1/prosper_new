...
class AccountDayBookReport(models.TransientModel):
    ...
    def check_report_html(self):
        ...
        return self.env.ref(
            'addis_systems_accounting_excel_reports.action_report_day_book_html').report_action(self,
                                                                     data=data)
    def check_report_excel(self):
        ...
        comparison_context = self._build_comparison_context(data)
        data['form']['comparison_context'] = comparison_context
        data=self.with_context(discard_logo_check=True,active_model='account.daybook.report',active_id=self.id).env["report.om_account_daily_reports.report_daybook"]._get_report_values(self,data)
        return  {
            'type': 'ir.actions.report',
            'report_type': 'excel',
            'data': {
                'model': 'account.daybook.report',
                'options': json.dumps(data, default=date_utils.json_default),
                'output_format': 'xlsx',
                'report_name': f"Day Book"
            }
        }
