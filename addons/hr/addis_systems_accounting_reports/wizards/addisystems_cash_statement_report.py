...
class AddissystemsAccountBalanceReport(models.TransientModel):
    _name="addisystems.cash.statement.report"
    ...
    def get_accounts(self,categ):
        account_map= {
            "operation_accounts":["asset_receivable","liability_payable"],
            "investing_accounts":["asset_fixed"],
            "financing_accounts":["equity"]
        }
        domain= self.get_constant_domain()
        domain+=[('account_id.account_type', '=', account_map[categ])]
    ...
    def check_report(self):
        return self.env.ref("addis_systems_accounting_reports.addis_systems_cash_flow_summary_report").report_action(self)
